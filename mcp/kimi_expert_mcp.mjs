...
const DEFAULT_CWD = process.env.CONTINUE_WORKSPACE || process.cwd();
const KIMI = process.env.KIMI_BIN || 'kimi';
const ROLE_FILE = process.env.KIMI_ROLE_FILE || path.join(DEFAULT_CWD, 'kimi_expert_role.md');
...
function globalContextCapsule() {
  const pipelinePath = path.join(DEFAULT_CWD, '调度状态', 'r3_pipeline_state.json');
  const workingPath = path.join(DEFAULT_CWD, '调度状态', 'working_loop_state.json');
  const pipeline = readJson(pipelinePath);
  const working = readJson(workingPath);
  const reportPath = pipeline.gate_report ? path.join(DEFAULT_CWD, pipeline.gate_report) : '';
