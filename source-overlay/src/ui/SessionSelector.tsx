  ...
  if (sessions.length === 0) {
    return (
      <Box {...defaultBoxStyles("blue")}>
        <Text color="yellow">没有找到之前的会话。</Text>
        <Text color="gray">输入 cn 开始新对话</Text>
  ...
  return (
    <Box flexDirection="row" width={terminalWidth}>
      ...
      <Box {...defaultBoxStyles("blue")} width={listWidth}>
        ...
        <Text color="gray">↑/↓ 导航，回车选择，Esc 退出</Text>
        <Text> </Text>

        {hasMoreAbove && (
          <Text color="gray" italic>
            ⬆ 上方还有 {scrollOffset} 个会话…
          </Text>
        )}
        ...
        {hasMoreBelow && (
          <Text color="gray" italic>
            ⬇ 下方还有 {sessions.length - scrollOffset - displaySessions.length} 个会话…
          </Text>
        )}
      </Box>
