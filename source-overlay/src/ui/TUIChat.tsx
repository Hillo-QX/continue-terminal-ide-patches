...
import { getCurrentSession, getTotalSessionCost } from "../session.js";
...
import type { WorkspaceAction } from "./WorkspaceSelector.js";
...
const TUIChat: React.FC<TUIChatProps> = ({
  ...
  const {
    ...
    onShowWorkspaceSelector: () => navigateTo("workspace"),
  ...
  const handleSessionSelect = useCallback(
    ...
    [closeCurrentScreen, setShowIntroMessage, switchSession],
  );

  const handleWorkspaceSelect = useCallback(
    (workspacePath: string, action: WorkspaceAction) => {
      ...
      const environment = {
        ...process.env,
        CONTINUE_WORKSPACE_PATH: workspacePath,
      };
      if (action === "resume") {
        nextArgs.push("--resume");
        environment.CONTINUE_CLI_RESUME_SESSION_ID = getCurrentSession().sessionId;
      } else {
        delete environment.CONTINUE_CLI_RESUME_SESSION_ID;
      }

      spawn(process.execPath, [entry, ...nextArgs], {
        cwd: workspacePath,
        env: environment,
        stdio: "inherit",
      });
