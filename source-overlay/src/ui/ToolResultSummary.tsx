...
const ToolResultSummary: React.FC<ToolResultSummaryProps> = ({
  ...
  if (toolName === "Checklist") {
    return (
      <Box flexDirection="column">
        <Box marginBottom={1}>
          <Text color="dim">⎿ </Text>
          <Text color="blue">任务列表已更新</Text>
        </Box>
        <ChecklistDisplay content={`任务列表状态：\n${content}`} />
      </Box>
    );
  }
  ...
  const getSummary = () => {
    ...
    if (content.startsWith("Error")) {
      const lines = content.split("\n");
      return `错误：${lines[0]}${lines.length > 1 ? "…" : ""}`;
    }
    ...
    switch (toolName) {
      ...
      case "List":
        return `已列出 ${lines} 个项目`;

      case "Search":
        return `找到 ${lines} 个匹配项`;
