...
const extractRuleNames = (rules: any[] = []): string[] => {
  return rules.map((rule: any) =>
    typeof rule === "string" ? rule : rule?.name || "未知",
  );
};
...
const IntroMessage: React.FC<IntroMessageProps> = ({
  ...
  return (
    <Box flexDirection="column" paddingX={1} paddingY={1}>
      ...
      {organizationName && (
        <Text color="blue">
          <Text bold>组织：</Text> <Text color="white">{organizationName}</Text>
        </Text>
      )}
      ...
      {config && (
        <Text color="blue">
          <Text bold>配置：</Text> <Text color="white">{config.name}</Text>
        </Text>
      )}
      ...
      {model ? (
        ...
        <Text color="blue">
          <Text bold>模型：</Text> <Text color="dim">加载中…</Text>
        </Text>
      )}
