...
export function useChat({
  ...
  const [currentSession, setCurrentSession] = useState<Session>(() => {
    ...
    const requestedSessionId = process.env.CONTINUE_CLI_RESUME_SESSION_ID;
    if (requestedSessionId) {
      const savedSession = loadOrCreateSessionById(requestedSessionId);
      savedSession.workspaceDirectory = process.cwd();
      return savedSession;
    }
  ...
  const [isChatHistoryInitialized, setIsChatHistoryInitialized] = useState(
    ...
    isRemoteMode ||
      !!process.env.CONTINUE_CLI_RESUME_SESSION_ID ||
  ...
  const switchSession = async (sessionId: string): Promise<boolean> => {
    const session = loadOrCreateSessionById(sessionId);
    if (!session) return false;

    session.workspaceDirectory = process.cwd();
    setCurrentSession(session);
    await services.chatHistory.initialize(session, isRemoteMode);
