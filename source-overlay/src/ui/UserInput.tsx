...
import { parseDraggedFilePaths } from "../util/draggedFilePaths.js";
...
const InterruptedBanner: React.FC<{ wasInterrupted: boolean }> = ({
  ...
  return (
    <Box paddingX={1} marginBottom={0}>
      <Text color="yellow">已暂停，按回车继续</Text>
    </Box>
  );
};
...
const UserInput: React.FC<UserInputProps> = ({
  ...
  const [slashCommands, setSlashCommands] = useState<SlashCommand[]>([
    ...
    { name: "exit", description: "退出对话", category: "system" },
  ]);
  ...
  const attachDraggedFiles = async (filePaths: string[]) => {
    ...
    for (const filePath of filePaths) {
      ...
      try {
        ...
        if (imageExtensions.has(extension)) {
          const placeholder = `__CONTINUE_DROPPED_IMAGE_${imageMap.size}__`;
          imageMap.set(placeholder, buffer);
          onFileAttached?.(filePath, `图片附件：${filePath}`);
        } else {
          onFileAttached?.(filePath, buffer.toString("utf8"));
        }
      } catch {
        onFileAttached?.(filePath, `文件附件：${filePath}`);
      }
    }

    const imagePlaceholders = [...imageMap.keys()].join(" ");
    const submittedText = `请处理文件：${names.join("、")}${imagePlaceholders ? ` ${imagePlaceholders}` : ""}`;
  ...
  const handleEnterKey = (key: any): boolean => {
    if (key.return && !key.shift) {
      ...
      if (textBuffer.text.trim() || wasInterrupted) {
        const draggedPaths = parseDraggedFilePaths(textBuffer.text);
        if (draggedPaths.length > 0) {
          void attachDraggedFiles(draggedPaths);
          return true;
        }
