...
const getServerStatusDisplay = (conn: MCPConnectionInfo) => {
  ...
  if (conn.status === "connecting") {
    ...
    if (conn.warnings && conn.warnings.length > 0) {
      icon = "🟡";
      color = "yellow";
      statusText = "已连接（有警告）" as MCPServerStatus;
    } else {
      icon = "🟢";
      color = "green";
    }
...
export const MCPSelector: React.FC<MCPSelectorProps> = ({ onCancel }) => {
  ...
  const backItem = { label: "返回", value: "back" };
  ...
  const getMainMenuItems = (): MCPMenuItem[] => {
    ...
    if (connections.length > 0) {
      ...
      // Add bulk actions
      options.push(
        { label: "🔄 重启所有服务器", value: "restart-all" },
        { label: "⏹️  停止所有服务器", value: "stop-all" },
      );
    }
  ...
  const getServerDetailItems = (): MCPMenuItem[] => {
    ...
    if (serverInfo.status === "connected") {
      items.push({ label: "⏹️  停止服务器", value: "stop" });
    }
    ...
    return (
      <Box flexDirection="column">
        ...
        {serverInfo.prompts.length > 0 && (
          <Box flexDirection="column" marginBottom={1}>
            <Text color="blue">📝 提示词：{serverInfo.prompts.length}</Text>
        ...
        {serverInfo.tools.length > 0 && (
          <Box flexDirection="column" marginBottom={1}>
            <Text color="blue">🔧 工具：{serverInfo.tools.length}</Text>
    ...
    try {
      switch (action) {
        case "restart":
          ...
          setMessage(`服务器“${selectedServer}”已重启`);
          break;
        case "stop":
          if (!selectedServer) {
            return;
          ...
          setMessage(`服务器“${selectedServer}”已停止`);
          break;
        case "back":
          handleBack();
          return;
  ...
  return (
    <Box {...defaultBoxStyles("blue", { minHeight: 10 })}>
      ...
      {message && (
        <Box marginBottom={1}>
          <Text color={message.startsWith("错误：") ? "red" : "green"}>
            {message}
          </Text>
        </Box>
      )}
