  ...
  implements ServiceWithDependencies
{
  ...
  private connectionInitPromise: Promise<void> | null = null;
  ...
  async doInitialize(
    ...
    const connectionInit = Promise.all(connectionPromises ?? []).then(() => undefined);
    this.connectionInitPromise = connectionInit;
  ...
  /**
   * Interactive mode starts MCP connections in the background so the TUI can
   * render immediately. Tool discovery must wait briefly before the first
   * request, otherwise the first request is sent with an empty MCP tool list.
   */
  public async waitForConnections(timeoutMs = 10000): Promise<void> {
    const pending = this.connectionInitPromise;
    if (!pending) return;
